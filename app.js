...
(() => {
  ...
  var cloudflareConfig = Object.freeze({
    auth: "mechpro-saas",
    apiUrl: "/api",
    authEndpoints: {
  ...
  function buildHomeModel({
    ...
    const open = orders2.filter((order) => !CLOSED.has(order.status));
    const inProgress = orders2.filter((order) => order.status === "in_progress");
    const waiting = orders2.filter((order) => order.status === "waiting_parts");
    const overdue = invoices2.filter((invoice) => invoice.status === "overdue");
    const unassigned = open.filter((order) => !order.tech || order.tech === "Unassigned");
    const todayKey = localIsoDate(now2);
    ...
    const attention = [
      ...waiting.map((order) => ({
        ...
        detail: `${order.customer} \xB7 ${order.vehicle}`,
        orderId: order.id
      })),
      ...
      ...overdue.map((invoice) => ({
        ...
        detail: `${invoice.customer} \xB7 ${Number(invoice.amount || 0)}`,
    ...
    return {
      ...
      paidTotal: invoices2.filter((invoice) => invoice.status === "paid").reduce((sum, invoice) => sum + Number(invoice.amount || 0), 0),
      todayJobs,
      todaysAppointments,
  ...
  var { buildHomeModel: buildHomeModel2, emptyState: emptyState2, escapeHtml: escapeHtml2, greetingForNow: greetingForNow2, localIsoDate: localIsoDate2, mergeRemoteCollection: mergeRemoteCollection2 } = window.__MECHPRO_HOME__;
  ...
  function openGroupChat(conversation = null) {
    ...
    showModal(`<form class="modal" id="group-chat-form"><div class="modal-head"><h2>${conversation ? "Manage group" : "New group chat"}</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><label>Group name<input name="name" maxlength="80" required value="${escapeHtml2(conversation?.name || "")}" placeholder="e.g. Service team"/></label><fieldset class="chat-member-picker"><legend>Members</legend>${people.map((user) => `<label><input type="checkbox" name="members" value="${escapeHtml2(user.email)}" ${selected.has(cleanEmail(user.email)) ? "checked" : ""} ${user.id === currentUser().id ? "disabled" : ""}/><span class="avatar">${initials(user.name)}</span><span><strong>${escapeHtml2(user.name)}</strong><small>${escapeHtml2(user.title || roleLabel[user.role])}</small></span></label>`).join("")}</fieldset></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">${icon("users", 14)} ${conversation ? "Save members" : "Create group"}</button></div></form>`);
  ...
  function liveAssistant() {
    const rows = assistantConversation.map((item) => `<article class="assistant-message ${item.role}"><strong>${item.role === "user" ? "You" : "MechPro Assistant"}</strong><p>${escapeHtml2(item.content)}</p></article>`).join("");
  ...
  var seed = {
    ...
    reminders: [],
    appointments: [
      { id: "apt-1048", sampleData: true, customer: "Demo Customer A", vehicle: "2018 Ford F-150 XLT", date: "2026-09-09", time: "09:00", service: "Brake inspection follow-up", tech: "Tech A", bay: "Bay 2", status: "confirmed" },
      { id: "apt-1049", sampleData: true, customer: "Demo Fleet Co", vehicle: "2021 Chevrolet Express 2500", date: "2026-09-09", time: "10:30", service: "Mobile no-crank diagnosis", tech: "Tech A", bay: "Mobile", status: "scheduled" },
  ...
  var cloudflareConfig2 = window.__MECHPRO_CONFIG__.cloudflare;
  ...
  async function cloudflareAccessSignIn() {
    ...
    if (!response.ok || !session.claims) throw new Error(session.message || "Cloudflare Access authentication failed");
  ...
  function authSession() {
    try {
      ...
      const expiresAt = Number(session.expiresAt || session.claims.exp * 1e3 || 0);
  ...
  async function verifyDesktopEntitlement() {
    if (!isDesktopApp2 || isLocalShell() && cloudflareConfig2.apiUrl.startsWith("/")) {
  ...
  function customerOptions(selected = "") {
    return state.customers.map((customer) => `<option ${customer.name === selected ? "selected" : ""}>${escapeHtml2(customer.name)}</option>`).join("");
  }
  ...
  function operationsVehicles() {
    const rows = state.vehicles.map((vehicle) => {
      ...
      return `<tr data-edit-vehicle="${vehicle.id}" class="clickable-row"><td><b>${escapeHtml2(vehicleLabel(vehicle))}</b><small>${escapeHtml2(vehicle.plate || "No plate")} \xB7 ${escapeHtml2(vehicle.mileage || "Mileage pending")}</small></td><td>${escapeHtml2(vehicle.customer)}</td><td class="mono">${escapeHtml2(vehicle.vin || "VIN pending")}</td><td>${history.length}<small>${history.at(-1)?.id || "No service yet"}</small></td><td>${vehicle.nextServiceDate || "Not set"}</td></tr>`;
    }).join("");
  ...
  function operationsInspections() {
    const rows = [...state.inspections].reverse().map((item) => {
      ...
      return `<tr><td><b>${escapeHtml2(item.number)}</b><small>${new Date(item.createdAt).toLocaleDateString()}</small></td><td>${escapeHtml2(item.customer)}<small>${escapeHtml2(item.vehicle)}</small></td><td>${escapeHtml2(item.workOrderId || "Unlinked")}</td><td><span class="inspection-count good">${counts.pass || 0} pass</span> <span class="inspection-count warn">${counts.attention || 0} attention</span> <span class="inspection-count fail">${counts.fail || 0} fail</span></td><td>${approvalBadge}</td><td><button class="mini-action" data-edit-inspection="${item.id}">${icon("clipboard-check", 13)} Open</button></td></tr>`;
    }).join(""), templateRows = state.inspectionTemplates.map((tpl) => `<tr><td><b>${escapeHtml2(tpl.name)}</b></td><td>${(tpl.items || []).length} points</td><td><button class="mini-action" data-edit-template="${tpl.id}">${icon("pencil", 13)} Edit</button></td></tr>`).join("");
  ...
  function operationsInventory() {
    const rows = state.inventory.map((item) => `<tr class="${Number(item.quantity) <= Number(item.reorderLevel) ? "low-stock" : ""}"><td><b>${escapeHtml2(item.name)}</b><small>${escapeHtml2(item.sku)} \xB7 ${escapeHtml2(item.kind)}</small></td><td>${Number(item.quantity || 0)} ${escapeHtml2(item.unit || "ea")}<small>Reorder at ${Number(item.reorderLevel || 0)}</small></td><td>${money(Number(item.cost || 0))}</td><td>${money(Number(item.price || 0))}</td><td>${escapeHtml2(item.vendor || "Unassigned")}</td><td><button class="mini-action" data-receive-stock="${item.id}">${icon("package-plus", 13)} Receive</button></td></tr>`).join("");
  ...
  function operationsServices() {
    const rows = state.services.map((item) => `<tr><td><b>${escapeHtml2(item.name)}</b><small>${escapeHtml2(item.description || "")}</small></td><td>${Number(item.laborHours || 0).toFixed(2)} hr</td><td>${money(Number(item.partsPrice || 0))}</td><td>${Number(item.discount || 0)}%</td></tr>`).join("");
  ...
  function operationsReminders() {
    const filters = [["all", "All"], ["due", "Due"], ["overdue", "Overdue"], ["sent", "Sent"]], items = [...state.reminders].sort((a, b) => String(a.dueDate).localeCompare(String(b.dueDate))).filter((item) => reminderFilter === "all" || reminderStatus(item) === reminderFilter), rows = items.map((item) => {
      ...
      return `<tr class="clickable-row" data-edit-reminder="${item.id}"><td>${escapeHtml2(item.customer)}<small>${escapeHtml2(item.vehicle)}</small></td><td>${escapeHtml2(item.service)}${item.parentReminderId ? `<small>Follow-up reminder</small>` : ""}</td><td>${item.dueDate || "Date pending"}<small>${item.dueMileage ? `${item.dueMileage} miles${vehicle?.mileage ? ` \xB7 current ${vehicle.mileage}` : ""}` : "No mileage trigger"}</small></td><td><span class="badge ${statusClass}">${statusLabel2}</span>${item.sentAt ? `<small>${new Date(item.sentAt).toLocaleString()}</small>` : ""}</td><td><div class="reminder-actions"><button class="mini-action" data-edit-reminder-button="${item.id}">${icon("pencil", 13)} Edit</button>${item.sentAt ? `<button class="mini-action" data-follow-up-reminder="${item.id}">${icon("calendar-plus", 13)} Follow up</button>` : `<button class="mini-action" data-send-reminder="${item.id}">${icon("send", 13)} Send</button>`}</div></td></tr>`;
    }).join("");
  ...
  function openVehicleForm(existing = null) {
    showModal(`<form class="modal" id="vehicle-form"><div class="modal-head"><h2>${existing ? "Edit" : "Add linked"} vehicle</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Owner<select name="customer" required>${customerOptions(existing?.customer)}</select></label><label>VIN<input name="vin" maxlength="17" pattern="[A-HJ-NPR-Z0-9]{17}" value="${escapeHtml2(existing?.vin || "")}"/></label><div class="full vin-actions"><button class="secondary" type="button" id="decode-vin">${icon("scan-line", 14)} Decode VIN with NHTSA</button><span id="vin-status"></span></div><label>Year<input name="year" required value="${escapeHtml2(existing?.year || "")}"/></label><label>Make<input name="make" required value="${escapeHtml2(existing?.make || "")}"/></label><label>Model<input name="model" required value="${escapeHtml2(existing?.model || "")}"/></label><label>Trim<input name="trim" value="${escapeHtml2(existing?.trim || "")}"/></label><label>Plate<input name="plate" value="${escapeHtml2(existing?.plate || "")}"/></label><label>Mileage<input name="mileage" type="number" min="0" value="${existing?.mileage || ""}"/></label><label>Next service date<input name="nextServiceDate" type="date" value="${existing?.nextServiceDate || ""}"/></label><label class="full">Notes<textarea name="notes">${escapeHtml2(existing?.notes || "")}</textarea></label><label class="full">Vehicle photos<input name="photos" type="file" accept="image/*" multiple/></label>${(existing?.photoKeys || []).length ? `<div class="full vehicle-photo-thumbs">${existing.photoKeys.map((key) => `<img src="${cloudflareConfig2.apiUrl}/files/${encodeURIComponent(key)}" alt="Vehicle photo" class="vehicle-thumb"/>`).join("")}</div>` : ""}</div></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary">Save vehicle</button></div></form>`);
    ...
    form.onsubmit = async (event) => {
      ...
      try {
        for (const file of files) {
          const key = await uploadFileToR2(file, "vehicle-photo", file.type);
          photoKeys.push(key);
        }
  ...
  function openManageTemplates() {
    const rows = state.inspectionTemplates.map((tpl) => `<tr><td><b>${escapeHtml2(tpl.name)}</b></td><td>${(tpl.items || []).length} points</td><td><button class="mini-action" data-tpl-edit="${tpl.id}">${icon("pencil", 13)} Edit</button></td></tr>`).join("");
  ...
  function inspectionApprovalHistory(inspection) {
    return (inspection.approvalHistory || []).map((entry) => `<div class="approval-entry"><span class="badge ${entry.status === "approved" ? "paid" : entry.status === "rejected" ? "overdue" : "estimate"}">${escapeHtml2(entry.status)}</span><span>${escapeHtml2(entry.actorName || "Unknown")} \xB7 ${new Date(entry.timestamp).toLocaleString()}</span>${entry.note ? `<small>${escapeHtml2(entry.note)}</small>` : ""}</div>`).join("");
  }
  function printInspectionReport(inspection) {
    const profile = shopProfile(), counts = (inspection.items || []).reduce((t, r) => (t[r.status] = (t[r.status] || 0) + 1, t), {}), itemRows = (inspection.items || []).map((item) => `<tr><td>${escapeHtml2(item.name)}</td><td class="${item.status}">${item.status === "pass" ? "Pass" : item.status === "attention" ? "Attention" : item.status === "fail" ? "Fail" : "Not checked"}</td><td>${escapeHtml2(item.note || "")}</td></tr>`).join(""), photoHtml = (inspection.photoKeys || []).map((key) => `<img src="${cloudflareConfig2.apiUrl}/files/${encodeURIComponent(key)}" style="width:120px;height:90px;object-fit:cover;border-radius:4px;border:1px solid #ddd"/>`).join(" "), damageHtml = (inspection.damageZones || []).length ? `<p><b>Damage zones:</b> ${inspection.damageZones.map(escapeHtml2).join(", ")}</p>` : "", approvalHtml = (inspection.approvalHistory || []).map((entry) => `<tr><td>${escapeHtml2(entry.status)}</td><td>${escapeHtml2(entry.actorName || "")}</td><td>${new Date(entry.timestamp).toLocaleString()}</td><td>${escapeHtml2(entry.note || "")}</td></tr>`).join(""), win = window.open("", "_blank");
    win.document.write(`<!DOCTYPE html><html><head><title>Inspection ${inspection.number}</title><style>body{font:12px/1.6 system-ui,sans-serif;max-width:900px;margin:auto;padding:20px}h1{font-size:20px}h2{font-size:16px;margin-top:18px}table{width:100%;border-collapse:collapse;margin:10px 0}th,td{padding:5px 8px;border:1px solid #ddd;text-align:left;font-size:11px}th{background:#f5f5f5}.pass{color:#087e6a;font-weight:700}.attention{color:#e2a524;font-weight:700}.fail{color:#d64b4b;font-weight:700}.not_checked{color:#999}img{margin:4px}@media print{button{display:none}}</style></head><body><h1>${escapeHtml2(profile.shopName)}</h1><p>${escapeHtml2(profile.address || "")} \xB7 ${escapeHtml2(profile.phone || "")}</p><h2>Digital Inspection Report: ${escapeHtml2(inspection.number)}</h2><p><b>Vehicle:</b> ${escapeHtml2(inspection.vehicle)} \xB7 <b>Customer:</b> ${escapeHtml2(inspection.customer)}</p><p><b>Date:</b> ${new Date(inspection.createdAt).toLocaleDateString()} \xB7 <b>Work Order:</b> ${escapeHtml2(inspection.workOrderId || "Unlinked")}</p><p><b>Results:</b> ${counts.pass || 0} pass \xB7 ${counts.attention || 0} attention \xB7 ${counts.fail || 0} fail</p>${damageHtml}<table><thead><tr><th>Item</th><th>Status</th><th>Notes</th></tr></thead><tbody>${itemRows}</tbody></table>${inspection.recommendations ? `<h2>Recommendations</h2><p>${escapeHtml2(inspection.recommendations)}</p>` : ""} ${photoHtml ? `<h2>Photos</h2><div>${photoHtml}</div>` : ""} ${approvalHtml ? `<h2>Approval History</h2><table><thead><tr><th>Decision</th><th>By</th><th>Date</th><th>Note</th></tr></thead><tbody>${approvalHtml}</tbody></table>` : ""}<p style="margin-top:18px;color:#999">Generated ${(/* @__PURE__ */ new Date()).toLocaleString()}</p><button onclick="window.print()">Print</button></body></html>`);
    win.document.close();
  }
...
` : ""}Date: ${new Date(inspection.createdAt).toLocaleDateString()}`;
  ...
  function openInspectionForm(existing = null) {
    const vehicleOptions = state.vehicles.map((vehicle) => `<option value="${escapeHtml2(vehicle.id)}" ${existing?.vehicleId === vehicle.id ? "selected" : ""}>${escapeHtml2(vehicle.customer)} \xB7 ${escapeHtml2(vehicleLabel(vehicle))}</option>`).join(""), templateOptions = `<option value="default">Default ${inspectionPoints.length}-point checklist</option>${state.inspectionTemplates.map((tpl) => `<option value="${escapeHtml2(tpl.id)}">${escapeHtml2(tpl.name)} (${(tpl.items || []).length} points)</option>`).join("")}`, items = existing?.items || inspectionPoints.map((name) => ({ name, status: "not_checked", note: "" })), zones = ["Front", "Rear", "Driver side", "Passenger side", "Roof", "Glass"], damage = new Set(existing?.damageZones || []), existingPhotos = (existing?.photoKeys || []).map((key) => `<img src="${cloudflareConfig2.apiUrl}/files/${encodeURIComponent(key)}" alt="Inspection photo" class="vehicle-thumb"/>`).join("");
    showModal(`<form class="modal wide" id="inspection-form"><div class="modal-head"><h2>${existing ? "Edit" : "New"} digital inspection</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Vehicle<select name="vehicleId" required>${vehicleOptions}</select></label><label>Work order<select name="workOrderId"><option value="">Unlinked</option>${state.orders.map((order) => `<option ${existing?.workOrderId === order.id ? "selected" : ""}>${order.id}</option>`).join("")}</select></label>${!existing ? `<label>Template<select name="templateId" id="template-picker">${templateOptions}</select></label>` : ""}</div><h3>Damage diagram</h3><div class="damage-zones">${zones.map((zone) => `<button type="button" class="damage-zone ${damage.has(zone) ? "marked" : ""}" data-damage-zone="${zone}">${icon(damage.has(zone) ? "alert-triangle" : "check", 13)} ${zone}</button>`).join("")}</div><h3 id="checklist-heading">${items.length}-point checklist</h3><div class="inspection-grid" id="inspection-checklist">${items.map((item, index) => `<article><b>${escapeHtml2(item.name)}</b><select name="status-${index}"><option value="not_checked" ${item.status === "not_checked" ? "selected" : ""}>Not checked</option><option value="pass" ${item.status === "pass" ? "selected" : ""}>Pass</option><option value="attention" ${item.status === "attention" ? "selected" : ""}>Needs attention</option><option value="fail" ${item.status === "fail" ? "selected" : ""}>Fail</option></select><input name="note-${index}" value="${escapeHtml2(item.note || "")}" placeholder="Reading or note"/></article>`).join("")}</div><label>Inspection photos<input name="photos" type="file" accept="image/*" multiple/></label>${existingPhotos ? `<div class="vehicle-photo-thumbs">${existingPhotos}</div>` : ""}<label>Recommendations<textarea name="recommendations">${escapeHtml2(existing?.recommendations || "")}</textarea></label>${existing ? `<div class="inspection-report-actions"><button type="button" class="secondary" id="print-inspection">${icon("printer", 14)} Print report</button><button type="button" class="secondary" id="share-inspection">${icon("share-2", 14)} Share report</button><button type="button" class="secondary" id="approve-inspection">${icon("shield-check", 14)} Approval</button></div>${inspectionApprovalHistory(existing) ? `<h3>Approval history</h3>${inspectionApprovalHistory(existing)}` : ""}` : ""}</div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary">${icon("save", 14)} Save inspection</button></div></form>`);
    ...
    document.querySelector("#template-picker")?.addEventListener("change", (event) => {
      ...
      grid.innerHTML = newItems.map((item, index) => `<article><b>${escapeHtml2(item.name)}</b><select name="status-${index}"><option value="not_checked" selected>Not checked</option><option value="pass">Pass</option><option value="attention">Needs attention</option><option value="fail">Fail</option></select><input name="note-${index}" placeholder="Reading or note"/></article>`).join("");
    });
...
${lines.join("\n")}`, raw: rawResponses.join("\n\n") };
  ...
  function superAdmin() {
    ...
    const accounts = platformAccounts || [], users = accounts.reduce((sum, account) => sum + account.users.length, 0), credits = accounts.reduce((sum, account) => sum + Number(account.creditBalance || 0), 0), rows = accounts.map((account) => `<tr><td><b>${escapeHtml2(account.shopName)}</b><small class="mono">${escapeHtml2(account.shopId)}</small></td><td><b>${escapeHtml2(account.ownerName)}</b><small>${escapeHtml2(account.ownerEmail)}</small></td><td><span class="badge paid">${account.users.length} login${account.users.length === 1 ? "" : "s"}</span><small>${account.users.map((user) => `${escapeHtml2(user.email)} \xB7 ${escapeHtml2(user.status || "Unknown")}`).join("<br>") || "No login found"}</small></td><td><b>${money(account.creditBalance || 0)}</b><small>Available account credit</small></td><td><div class="platform-row-actions"><button class="mini-action" data-credit-shop="${escapeHtml2(account.shopId)}" data-shop-name="${escapeHtml2(account.shopName)}">${icon("badge-dollar-sign", 13)} Give credit</button>${account.users.map((user) => `<button class="mini-action" data-reset-user="${encodeURIComponent(user.username || user.email)}">${icon("key-round", 13)} Reset password</button>`).join("")}</div></td></tr>`).join("");
  ...
  function bindLiveAssistant() {
    ...
    const input = form.elements.message, sendButton = form.querySelector("button[type=submit]"), append = (role, content) => {
      ...
      list.innerHTML = assistantConversation.map((item) => `<article class="assistant-message ${item.role}"><strong>${item.role === "user" ? "You" : "MechPro Assistant"}</strong><p>${escapeHtml2(item.content)}</p></article>`).join("");
...
AI workflow: ${aiResult.diagnostics.causes[0]?.cause || "Inspection required"}`.trim();
  ...
  function newOrderEstimateRow(line = { service: "Custom service", notes: "Describe the inspection, labor, parts, and verification included with this service.", hours: 1, parts: 0 }) {
    return `<article class="new-estimate-line"><div class="estimate-line-head"><strong>Service line</strong><button class="icon-button remove-estimate-line" type="button" title="Remove service">${icon("trash-2", 14)}</button></div><label>Service<input class="estimate-service" value="${escapeHtml2(line.service)}" required/></label><label>What this service includes<textarea class="estimate-explanation" required>${escapeHtml2(line.notes)}</textarea></label><div class="estimate-line-numbers"><label>Labor hours<input class="estimate-hours" type="number" min="0" step=".25" value="${Number(line.hours || 0)}"/></label><label>Parts & materials<input class="estimate-parts" type="number" min="0" step=".01" value="${Number(line.parts || 0).toFixed(2)}"/></label><div><span>Line total</span><b class="estimate-line-total">${money(Number(line.hours || 0) * 165 + Number(line.parts || 0))}</b></div></div></article>`;
  }
  ...
  function techOptions(selected = "Unassigned") {
    ...
    return [...new Set(names)].map((name) => `<option ${name === selected ? "selected" : ""}>${escapeHtml2(name)}</option>`).join("");
  }
  function vehicleOptionsForCustomer(customerName, selectedVin = "") {
    ...
    return `<option value="">Enter vehicle manually</option>${vehicles.map((vehicle) => `<option value="${escapeHtml2(vehicle.id)}" data-vin="${escapeHtml2(vehicle.vin || "")}" data-label="${escapeHtml2(vehicleLabel(vehicle))}" ${vehicle.vin && vehicle.vin === selectedVin ? "selected" : ""}>${escapeHtml2(vehicleLabel(vehicle))}${vehicle.vin ? ` \xB7 ${escapeHtml2(vehicle.vin)}` : ""}</option>`).join("")}`;
  }
  ...
  function openOrder(id) {
    ...
    const technicians = state.users.filter((user) => user.active && user.techName).map((user) => user.techName), canManage = ["admin", "service_writer"].includes(currentUser().role), isAssignedTech = currentUser().role === "technician" && currentUser().techName === x.tech, activeClock = openJobClock(x.id), jobClockControl = isAssignedTech ? `<section class="job-clock-panel"><div><h3>Job time clock</h3><p>${activeClock ? `Clocked in at ${formatTime(activeClock.clockIn)}` : `${formatHours(jobTrackedHours(x.id))} tracked on this job`}</p></div><button class="${activeClock ? "secondary danger" : "primary"}" id="job-clock" data-work-order-id="${x.id}">${icon(activeClock ? "square" : "play", 14)} ${activeClock ? "Clock out job" : "Clock in to job"}</button></section>` : "", inventoryLines = (x.estimate?.lines || []).filter((line) => line.inventoryId || line.inventorySku), inventoryMarkup = inventoryLines.length ? `<section><h3>Linked inventory</h3>${inventoryLines.map((line) => {
      const item = state.inventory.find((record) => record.id === line.inventoryId || line.inventorySku && record.sku === line.inventorySku);
      return `<p><b>${escapeHtml2(item?.sku || line.inventorySku || "Unknown SKU")}</b> \xB7 ${escapeHtml2(item?.name || line.service)} \xB7 ${Number(line.committedQuantity || 0)} committed</p>`;
  ...
  function scheduleWorkspace() {
    const rows = [...state.appointments].sort((a, b) => `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`)).map((item) => `<tr class="clickable-row" data-edit-appointment="${item.id}"><td><b>${escapeHtml2(item.date)}</b><small>${escapeHtml2(item.time)}</small></td><td>${escapeHtml2(item.customer)}<small>${escapeHtml2(item.vehicle)}</small></td><td>${escapeHtml2(item.service)}</td><td>${escapeHtml2(item.tech || "Unassigned")}<small>${escapeHtml2(item.bay || "Unassigned")}</small></td><td><span class="badge ${item.status === "completed" ? "paid" : "approved"}">${escapeHtml2(item.status)}</span></td><td><button class="mini-action" data-edit-appointment-button="${item.id}">${icon("pencil", 13)} Edit</button></td></tr>`).join("");
  ...
  function openAppointmentForm(existing = null) {
    ...
    showModal(`<form class="modal" id="appointment-form"><div class="modal-head"><h2>${existing ? "Edit" : "New"} appointment</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Customer<select name="customer">${customerOptions(existing?.customer)}</select></label><label>Vehicle<input name="vehicle" required value="${escapeHtml2(existing?.vehicle || "")}"/></label><label>Date<input name="date" type="date" required value="${existing?.date || ""}"/></label><label>Time<input name="time" type="time" required value="${existing?.time || ""}"/></label><label>Service<input name="service" required value="${escapeHtml2(existing?.service || "")}"/></label><label>Technician<input name="tech" value="${escapeHtml2(existing?.tech || "")}"/></label><label>Bay / mobile<input name="bay" value="${escapeHtml2(existing?.bay || "")}"/></label><label>Status<select name="status">${statuses.map((status) => `<option ${existing?.status === status ? "selected" : ""}>${status}</option>`).join("")}</select></label></div></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary">Save appointment</button></div></form>`);
  ...
  function operationsAnalytics() {
    ...
    return `<div class="finance-kpis"><article><span>Cash received</span><strong>${money(revenue)}</strong></article><article><span>Open receivables</span><strong>${money(finance().receivable)}</strong></article><article><span>Inventory value</span><strong>${money(inventoryValue)}</strong></article><article><span>Low stock</span><strong>${state.inventory.filter((item) => Number(item.quantity) <= Number(item.reorderLevel)).length}</strong></article><article><span>Reminders due</span><strong>${state.reminders.filter((item) => !item.sentAt).length}</strong></article></div><div class="accounting-grid"><section class="statement"><div class="statement-head"><h2>Technician productivity</h2></div>${techs.map((tech) => `<div class="statement-line"><span>${escapeHtml2(tech.name)} \xB7 ${tech.hours.toFixed(2)} tracked hr</span><b>${tech.jobs} completed</b></div>`).join("") || "No completed technician work."}</section><section class="statement"><div class="statement-head"><h2>Customer lifetime value</h2></div>${customers2.map((customer) => `<div class="statement-line"><span>${escapeHtml2(customer.name)}</span><b>${money(Number(customer.spend || 0))}</b></div>`).join("")}</section></div>`;
  }
  ...
  function openEstimateDiscount(id) {
    const estimate = state.estimates.find((item) => item.id === id);
    showModal(`<form class="modal" id="estimate-discount-form"><div class="modal-head"><h2>Estimate discount</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><label>Discount percent<input name="discount" type="number" min="0" max="100" step=".1" value="${Number(estimate.discountPercent || 0)}" required/></label><label>Reason<input name="discountReason" value="${escapeHtml2(estimate.discountReason || "")}"/></label></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary">Apply discount</button></div></form>`);
  ...
  settings = function() {
    ...
    return shell(`${heading("Administration", "Shop settings", "Branding, invoice defaults, tax, and optional provider readiness.", false)}<form class="settings-panel form-grid" id="shop-profile-form"><label>Shop name<input name="shopName" value="${escapeHtml2(profile.shopName)}" required/></label><label>Phone<input name="phone" value="${escapeHtml2(profile.phone)}"/></label><label class="full">Address<input name="address" value="${escapeHtml2(profile.address)}"/></label><label>Logo URL<input name="logoUrl" type="url" value="${escapeHtml2(profile.logoUrl)}"/></label><label>Default labor rate<input name="laborRate" type="number" step=".01" value="${Number(profile.laborRate)}"/></label><label class="full">Invoice footer<input name="invoiceFooter" value="${escapeHtml2(profile.invoiceFooter)}"/></label><div class="full service-contract"><h2>Commercial integrations</h2><p>CarFax service history and license-plate recognition require provider contracts and server-side credentials. They remain unavailable until configured by the platform operator.</p></div><label class="toggle-field"><input type="checkbox" disabled ${profile.carfaxEnabled ? "checked" : ""}/><span>CarFax provider configured</span></label><label class="toggle-field"><input type="checkbox" disabled ${profile.plateProviderEnabled ? "checked" : ""}/><span>Plate recognition configured</span></label><div class="full"><button class="primary">${icon("save", 14)} Save business profile</button></div></form><div class="settings-panel"><form class="form-grid" id="tax-settings-form"><label>Filing state<select name="state">${stateOptions}</select></label><label>State tax ID<input name="taxId" value="${escapeHtml2(t.taxId)}"/></label><label>Sales tax rate %<input name="rate" type="number" step=".01" min="0" value="${t.rate}"/></label><label>Filing frequency<select name="filingFrequency"><option ${t.filingFrequency === "Monthly" ? "selected" : ""}>Monthly</option><option ${t.filingFrequency === "Quarterly" ? "selected" : ""}>Quarterly</option><option ${t.filingFrequency === "Annually" ? "selected" : ""}>Annually</option></select></label><div class="full"><button class="primary">Save tax settings</button></div></form></div>`);
  };
  ...
  newOrderEstimateRow = function(line = { service: "Custom service", notes: "Describe the inspection, labor, parts, and verification included with this service.", hours: 1, parts: 0, discount: 0 }) {
    const inventoryOptions = state.inventory.map((item) => `<option value="${escapeHtml2(item.id)}" ${line.inventoryId === item.id ? "selected" : ""}>${escapeHtml2(item.sku || "No SKU")} \xB7 ${escapeHtml2(item.name)} (${Number(item.quantity || 0)} on hand)</option>`).join("");
    return `<article class="new-estimate-line"><div class="estimate-line-head"><strong>Service line</strong><button class="icon-button remove-estimate-line" type="button" title="Remove service">${icon("trash-2", 14)}</button></div><label>Service<input class="estimate-service" value="${escapeHtml2(line.service)}" required/></label><label>What this service includes<textarea class="estimate-explanation" required>${escapeHtml2(line.notes || line.description || "")}</textarea></label><div class="estimate-line-numbers"><label>Labor hours<input class="estimate-hours" type="number" min="0" step=".25" value="${Number(line.hours || line.laborHours || 0)}"/></label><label>Parts & materials<input class="estimate-parts" type="number" min="0" step=".01" value="${Number(line.parts ?? line.partsPrice ?? 0).toFixed(2)}"/></label><label>Discount %<input class="estimate-discount" type="number" min="0" max="100" step=".1" value="${Number(line.discount || 0)}"/></label><label>Inventory SKU<select class="estimate-inventory"><option value="">Not linked</option>${inventoryOptions}</select></label><label>Committed quantity<input class="estimate-committed-quantity" type="number" min=".01" step=".01" value="${Number(line.committedQuantity || 1)}"/></label><div><span>Line total</span><b class="estimate-line-total">${money((Number(line.hours || line.laborHours || 0) * 165 + Number(line.parts ?? line.partsPrice ?? 0)) * (1 - Number(line.discount || 0) / 100))}</b></div></div></article>`;
  };
  ...
  bindNewOrderEstimator = function() {
    ...
    lines.insertAdjacentHTML("beforebegin", `<div class="canned-service-picker"><span>Canned services</span>${state.services.map((service) => `<button type="button" class="mini-action" data-add-canned-service="${service.id}">${escapeHtml2(service.name)}</button>`).join("")}</div>`);
  ...
  function printCustomerStatement(name) {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Statement \u2014 ${name}</title><style>body{font:12px/1.6 system-ui,sans-serif;max-width:800px;margin:auto;padding:20px}h1{font-size:20px}table{width:100%;border-collapse:collapse;margin:12px 0}th,td{padding:6px 8px;border-bottom:1px solid #ddd;text-align:left;font-size:11px}th{background:#f5f5f5;font-weight:700}.total{font-weight:700;font-size:14px}@media print{button{display:none}}</style></head><body><h1>${escapeHtml2(profile.shopName)}</h1><p>${escapeHtml2(profile.address || "")} \xB7 ${escapeHtml2(profile.phone || "")}</p><h2>Customer Statement: ${escapeHtml2(name)}</h2><p>Generated ${(/* @__PURE__ */ new Date()).toLocaleDateString()}</p><h3>Invoices</h3><table><thead><tr><th>Invoice</th><th>Date</th><th>Status</th><th>Amount</th><th>Balance</th></tr></thead><tbody>${invs.map((i) => `<tr><td>${i.number}</td><td>${i.date}</td><td>${i.status}</td><td>$${i.amount.toFixed(2)}</td><td>$${invoiceBalance(i).toFixed(2)}</td></tr>`).join("")}</tbody></table><h3>Payments</h3><table><thead><tr><th>Date</th><th>Method</th><th>Amount</th><th>Invoice</th></tr></thead><tbody>${payments2.map((p) => `<tr><td>${p.receivedAt || ""}</td><td>${p.method || ""}</td><td>$${p.amount.toFixed(2)}</td><td>${p.invoiceNumber || ""}</td></tr>`).join("")}</tbody></table><p class="total">Account Balance: $${balance.toFixed(2)}</p><button onclick="window.print()">Print</button></body></html>`);
    win.document.close();
  }
  function openVehicleDetail(id) {
    ...
    showModal(`<div class="modal wide" id="vehicle-detail"><div class="modal-head"><h2>${escapeHtml2(vehicleLabel(vehicle))}</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Owner<input value="${escapeHtml2(vehicle.customer)}" disabled/></label><label>VIN<input value="${escapeHtml2(vehicle.vin || "")}" disabled/></label><label>Plate<input value="${escapeHtml2(vehicle.plate || "")}" disabled/></label><label>Mileage<input value="${vehicle.mileage || ""}" disabled/></label><label>Next service<input value="${vehicle.nextServiceDate || ""}" disabled/></label><label>Notes<input value="${escapeHtml2(vehicle.notes || "")}" disabled/></label></div>${photos.length ? `<h3>Photos</h3><div class="vehicle-photo-thumbs">${photos.map((key) => `<img src="${cloudflareConfig2.apiUrl}/files/${encodeURIComponent(key)}" alt="Vehicle photo" class="vehicle-thumb"/>`).join("")}</div>` : ""}<h3>Service History (${history.length})</h3><table class="mini-table"><thead><tr><th>RO</th><th>Status</th><th>Vehicle</th><th>Total</th></tr></thead><tbody>${history.map((o) => `<tr><td class="mono">${o.id}</td><td>${badge(o.status)}</td><td>${escapeHtml2(o.vehicle)}</td><td>${money(o.total)}</td></tr>`).join("") || `<tr><td colspan="4">No service history</td></tr>`}</tbody></table><div class="modal-actions"><button class="secondary" id="vd-edit">${icon("pencil", 14)} Edit vehicle</button></div></div></div>`);
  ...
  async function resolveAuthenticatedProfile(session) {
    ...
    if (role === "super_admin") {
      ...
      if (local) return Object.assign(local, { role: "super_admin", active: true });
  ...
  function pendingProfileScreen() {
    ...
    return `<main class="login-screen"><section class="login-panel"><div class="brand login-brand"><div class="brand-mark">${icon("wrench")}</div><div><div class="brand-name">MechPro</div><small>Account almost ready</small></div></div><div class="eyebrow">Employee profile required</div><h1>${pending.reason === "deactivated" ? "Your shop profile is deactivated" : "Ask an admin to finish setup"}</h1><p>${pending.reason === "deactivated" ? `The Cloudflare Access identity <strong>${escapeHtml2(pending.email || "")}</strong> works, but the matching employee profile is inactive.` : `You signed in as <strong>${escapeHtml2(pending.email || "")}</strong> (${escapeHtml2(roleLabel[pending.role] || pending.role || "team member")}), but this shop does not have an active employee profile for that email yet.`}</p><div class="service-contract"><p>Ask a shop administrator to open <b>Employees</b> and create (or reactivate) a profile using exactly this email:</p><code>${escapeHtml2(pending.email || "")}</code></div><div class="modal-actions" style="justify-content:flex-start;margin-top:18px"><button class="primary" id="pending-sign-out">${icon("log-out", 14)} Sign out</button></div></section></main>`;
  }
  ...
  bind = function() {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (isDesktopApp2) await verifyDesktopEntitlement();
        ...
        pendingAuthProfile = null;
        desktopEntitlementVerified = !isDesktopApp2;
        desktopLoginMessage = isDesktopApp2 ? error.message : "";
  ...
  settings = function() {
    const profile = shopProfile(), t = state.taxSettings, stateOptions = usStates.map((s) => `<option value="${s.code}" ${t.state === s.code ? "selected" : ""}>${s.name}</option>`).join(""), kinds = ["Part", "Tire", "Supply", "Asset"], couponRows = profile.coupons.map((coupon) => `<tr><td><b>${escapeHtml2(coupon.code)}</b></td><td>${coupon.percent}%</td><td><span class="badge ${coupon.active ? "paid" : "estimate"}">${coupon.active ? "Active" : "Inactive"}</span></td><td><div class="coupon-actions"><button type="button" class="mini-action" data-edit-coupon="${escapeHtml2(coupon.id)}">${icon("pencil", 13)} Edit</button><button type="button" class="mini-action" data-toggle-coupon="${escapeHtml2(coupon.id)}">${icon(coupon.active ? "pause" : "play", 13)} ${coupon.active ? "Disable" : "Enable"}</button><button type="button" class="mini-action danger" data-delete-coupon="${escapeHtml2(coupon.id)}">${icon("trash-2", 13)} Delete</button></div></td></tr>`).join("");
    return shell(`${heading("Administration", "Shop settings", "Branding, invoice defaults, coupons, vendors, tax, and optional provider readiness.", false)}<form class="settings-panel form-grid" id="shop-profile-form"><div class="full statement-head"><div><div class="eyebrow">Business identity</div><h2>Branding and invoice defaults</h2></div></div>${appearanceFieldset(profile)}<label>Shop name<input name="shopName" value="${escapeHtml2(profile.shopName)}" required/></label><label>Phone<input name="phone" value="${escapeHtml2(profile.phone)}"/></label><label class="full">Address<input name="address" value="${escapeHtml2(profile.address)}"/></label><label>Logo URL<input name="logoUrl" type="url" value="${escapeHtml2(profile.logoUrl)}" placeholder="https://example.com/logo.png"/></label><label>Default labor rate<input name="laborRate" type="number" min="0" step=".01" value="${Number(profile.laborRate)}"/></label><label>Brand color<input name="brandColor" type="color" value="${profile.brandColor}"/></label><label>Accent color<input name="accentColor" type="color" value="${profile.accentColor}"/></label><label class="full">Invoice footer<input name="invoiceFooter" value="${escapeHtml2(profile.invoiceFooter)}"/></label><div class="full"><button class="primary">${icon("save", 14)} Save business profile</button></div></form><section class="settings-panel"><div class="statement-head"><div><div class="eyebrow">Discounts</div><h2>Coupons</h2></div><button class="primary" type="button" id="add-coupon">${icon("badge-percent", 14)} Add coupon</button></div><div class="data-panel"><table><thead><tr><th>Code</th><th>Percent</th><th>Status</th><th>Actions</th></tr></thead><tbody>${couponRows || `<tr><td colspan="4">No coupons configured.</td></tr>`}</tbody></table></div></section><form class="settings-panel form-grid" id="vendor-defaults-form"><div class="full statement-head"><div><div class="eyebrow">Purchasing</div><h2>Default vendors</h2></div></div><label>Overall default<select name="defaultVendor">${vendorOptions(profile.defaultVendor)}</select></label>${kinds.map((kind) => `<label>${kind} default<select name="vendor${kind}">${vendorOptions(profile.defaultVendorByKind?.[kind] || "")}</select></label>`).join("")}<div class="full"><button class="primary">${icon("save", 14)} Save vendor defaults</button></div></form><div class="settings-panel"><form class="form-grid" id="tax-settings-form"><label>Filing state<select name="state">${stateOptions}</select></label><label>State tax ID<input name="taxId" value="${escapeHtml2(t.taxId)}"/></label><label>Sales tax rate %<input name="rate" type="number" step=".01" min="0" value="${t.rate}"/></label><label>Filing frequency<select name="filingFrequency"><option ${t.filingFrequency === "Monthly" ? "selected" : ""}>Monthly</option><option ${t.filingFrequency === "Quarterly" ? "selected" : ""}>Quarterly</option><option ${t.filingFrequency === "Annually" ? "selected" : ""}>Annually</option></select></label><div class="full"><button class="primary">${icon("save", 14)} Save tax settings</button></div></form></div>`);
  };
  function openCouponForm(id = "") {
    const profile = shopProfile(), coupon = profile.coupons.find((item) => item.id === id);
    showModal(`<form class="modal" id="coupon-form"><div class="modal-head"><h2>${coupon ? "Edit" : "Add"} coupon</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Code<input name="code" maxlength="24" pattern="[A-Za-z0-9_-]+" value="${escapeHtml2(coupon?.code || "")}" required/></label><label>Discount percent<input name="percent" type="number" min=".1" max="100" step=".1" value="${coupon?.percent || ""}" required/></label><label class="toggle-field full"><input name="active" type="checkbox" ${coupon?.active !== false ? "checked" : ""}/><span>Coupon is active</span></label></div></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary">Save coupon</button></div></form>`);
  ...
  openEstimateDiscount = function(id) {
    ...
    showModal(`<form class="modal" id="estimate-discount-form"><div class="modal-head"><h2>Estimate discount</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><label>Coupon<select name="couponCode" id="estimate-coupon"><option value="">Manual discount</option>${coupons.map((coupon) => `<option value="${escapeHtml2(coupon.code)}" data-percent="${coupon.percent}" ${estimate.couponCode === coupon.code ? "selected" : ""}>${escapeHtml2(coupon.code)} \xB7 ${coupon.percent}%</option>`).join("")}</select></label><label>Discount percent<input name="discount" type="number" min="0" max="100" step=".1" value="${Number(estimate.discountPercent || 0)}" required/></label><label>Reason<input name="discountReason" value="${escapeHtml2(estimate.discountReason || "")}"/></label></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary">Apply discount</button></div></form>`);
  ...
  function printInvoice(number) {
    const invoice = state.invoices.find((item) => item.number === number);
    if (!invoice) return;
    const profile = shopProfile(), brand = printableBrand(profile), breakdown = invoiceTaxBreakdown(invoice), paid = invoicePaid(invoice), balance = invoiceBalance(invoice), customer = state.customers.find((item) => item.name === invoice.customer), order = state.orders.find((item) => item.id === invoice.ro), payments2 = paymentRecords().filter((payment) => payment.invoiceNumber === invoice.number), paymentRows = payments2.map((payment) => `<tr><td>${escapeHtml2(payment.receivedAt || "")}</td><td>${escapeHtml2(payment.method || "")}</td><td>${escapeHtml2(payment.reference || "")}</td><td>${money(Number(payment.amount || 0))}</td></tr>`).join(""), win = window.open("", "_blank", "noopener");
    if (!win) {
      toast("Allow pop-ups to print the invoice");
      return;
    }
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapeHtml2(invoice.number)} invoice</title><style>${brand.style}body{font:12px/1.55 system-ui,sans-serif;max-width:850px;margin:auto;padding:22px;color:#172029}h2{margin:20px 0 8px}.invoice-meta{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:18px 0}.invoice-meta div,.totals{padding:10px;border:1px solid #d6ddd7}.invoice-meta b,.invoice-meta span{display:block}.invoice-meta span{color:#66727a}table{width:100%;border-collapse:collapse;margin:10px 0}th,td{padding:7px 9px;border-bottom:1px solid #d6ddd7;text-align:left}th{color:#fff;background:var(--brand)}.totals{width:min(320px,100%);margin:18px 0 18px auto}.totals p{display:flex;justify-content:space-between;margin:5px 0}.totals .balance{padding-top:8px;border-top:2px solid var(--brand);font-size:15px}.footer{margin-top:28px;padding-top:12px;border-top:1px solid #d6ddd7;text-align:center;color:#66727a}@media(max-width:600px){.invoice-meta{grid-template-columns:1fr}}</style></head><body>${brand.header}<div class="invoice-meta"><div><span>Invoice</span><b>${escapeHtml2(invoice.number)}</b></div><div><span>Repair order</span><b>${escapeHtml2(invoice.ro || "")}</b></div><div><span>Customer</span><b>${escapeHtml2(invoice.customer || "")}</b>${customer?.billingAddress ? `<small>${escapeHtml2(customer.billingAddress)}</small>` : ""}</div><div><span>Vehicle</span><b>${escapeHtml2(order?.vehicle || "")}</b></div><div><span>Issued</span><b>${escapeHtml2(invoice.date || "")}</b></div><div><span>Due</span><b>${escapeHtml2(invoice.due || "")}</b></div></div><h2>Invoice summary</h2><table><thead><tr><th>Description</th><th>Amount</th></tr></thead><tbody><tr><td>Services and materials for ${escapeHtml2(invoice.ro || invoice.number)}</td><td>${money(breakdown.subtotal)}</td></tr><tr><td>Sales tax (${Number(breakdown.taxRate || 0)}%)</td><td>${money(breakdown.tax)}</td></tr></tbody></table>${paymentRows ? `<h2>Payments</h2><table><thead><tr><th>Date</th><th>Method</th><th>Reference</th><th>Amount</th></tr></thead><tbody>${paymentRows}</tbody></table>` : ""}<div class="totals"><p><span>Subtotal</span><b>${money(breakdown.subtotal)}</b></p><p><span>Tax</span><b>${money(breakdown.tax)}</b></p><p><span>Invoice amount</span><b>${money(invoice.amount)}</b></p><p><span>Payments</span><b>-${money(paid)}</b></p><p class="balance"><span>Balance due</span><b>${money(balance)}</b></p></div><p class="footer">${escapeHtml2(profile.invoiceFooter || "")}</p><button type="button" onclick="window.print()">Print invoice</button></body></html>`);
    win.document.close();
  }
  invoices = function() {
    const q = query.toLowerCase(), rows = state.invoices.filter((invoice) => !q || Object.values(invoice).join(" ").toLowerCase().includes(q)).map((invoice) => {
      ...
      return `<tr><td class="mono"><b>${escapeHtml2(invoice.number)}</b></td><td class="mono">${escapeHtml2(invoice.ro || "")}</td><td><b>${escapeHtml2(invoice.customer)}</b></td><td>${escapeHtml2(invoice.date || "")}</td><td>${escapeHtml2(invoice.due || "")}</td><td>${balance === 0 ? `<span class="badge paid">Paid</span>` : badge(invoice.status)}</td><td><b>${money(invoice.amount)}</b><small>Subtotal ${money(bd.subtotal)} \xB7 Tax ${money(bd.tax)} (${bd.taxRate}%)</small><small>Paid ${money(paid)} \xB7 Balance ${money(balance)}</small><div class="invoice-payments"><button class="mini-action" data-print-invoice="${escapeHtml2(invoice.number)}">${icon("printer", 13)} Print</button>${balance > 0 ? `<button class="mini-action" data-record-payment="${escapeHtml2(invoice.number)}" data-method="cash">${icon("banknote", 13)} Cash</button><button class="mini-action" data-record-payment="${escapeHtml2(invoice.number)}" data-method="processor">${icon("credit-card", 13)} Card receipt</button><button class="mini-action invoice-pay" data-pay-invoice="${escapeHtml2(invoice.number)}">${icon("external-link", 13)} Pay online</button>` : ""}${paid === 0 ? `<button class="mini-action" data-edit-tax="${escapeHtml2(invoice.number)}">${icon("percent", 13)} Edit tax</button>` : ""}</div></td></tr>`;
    }).join("");
  ...
  bindEstimateActions = function() {
    ...
    document.querySelectorAll("[data-print-invoice]").forEach((button) => {
      ...
      if (!invoice) return;
      button.insertAdjacentHTML("afterend", `<button class="mini-action danger" data-delete-invoice="${escapeHtml2(invoiceRecordKey(invoice))}">${icon("trash-2", 13)} Delete</button>`);
  ...
  printCustomerStatement = function(name) {
    const invoices2 = state.invoices.filter((invoice) => invoice.customer === name), payments2 = paymentRecords().filter((payment) => payment.customer === name), balance = customerBalance(name), profile = shopProfile(), brand = printableBrand(profile), invoiceRows = invoices2.map((invoice) => `<tr><td>${escapeHtml2(invoice.number)}</td><td>${escapeHtml2(invoice.date || "")}</td><td>${escapeHtml2(invoice.status || "")}</td><td>${money(invoice.amount)}</td><td>${money(invoiceBalance(invoice))}</td></tr>`).join(""), paymentRows = payments2.map((payment) => `<tr><td>${escapeHtml2(payment.receivedAt || "")}</td><td>${escapeHtml2(payment.method || "")}</td><td>${money(Number(payment.amount || 0))}</td><td>${escapeHtml2(payment.invoiceNumber || "")}</td></tr>`).join(""), win = window.open("", "_blank", "noopener");
    if (!win) {
      toast("Allow pop-ups to print the statement");
      return;
    }
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapeHtml2(name)} statement</title><style>${brand.style}body{font:12px/1.6 system-ui,sans-serif;max-width:850px;margin:auto;padding:22px;color:#172029}h2{color:var(--brand)}table{width:100%;border-collapse:collapse;margin:12px 0}th,td{padding:7px 9px;border-bottom:1px solid #d6ddd7;text-align:left}th{color:#fff;background:var(--brand)}.total{margin-top:18px;padding:12px;border-top:4px solid var(--brand);font-size:16px;text-align:right}</style></head><body>${brand.header}<h2>Customer Statement: ${escapeHtml2(name)}</h2><p>Generated ${escapeHtml2((/* @__PURE__ */ new Date()).toLocaleDateString())}</p><h3>Invoices</h3><table><thead><tr><th>Invoice</th><th>Date</th><th>Status</th><th>Amount</th><th>Balance</th></tr></thead><tbody>${invoiceRows || `<tr><td colspan="5">No invoices.</td></tr>`}</tbody></table><h3>Payments</h3><table><thead><tr><th>Date</th><th>Method</th><th>Amount</th><th>Invoice</th></tr></thead><tbody>${paymentRows || `<tr><td colspan="4">No payments.</td></tr>`}</tbody></table><p class="total">Account Balance: ${money(balance)}</p><button type="button" onclick="window.print()">Print statement</button></body></html>`);
    win.document.close();
  };
  printInspectionReport = function(inspection) {
    const profile = shopProfile(), brand = printableBrand(profile), counts = (inspection.items || []).reduce((total, item) => (total[item.status] = (total[item.status] || 0) + 1, total), {}), itemRows = (inspection.items || []).map((item) => `<tr><td>${escapeHtml2(item.name)}</td><td>${escapeHtml2(item.status === "not_checked" ? "Not checked" : item.status || "")}</td><td>${escapeHtml2(item.note || "")}</td></tr>`).join(""), damage = (inspection.damageZones || []).map(escapeHtml2).join(", "), approvalRows = (inspection.approvalHistory || []).map((entry) => `<tr><td>${escapeHtml2(entry.status)}</td><td>${escapeHtml2(entry.actorName || "")}</td><td>${escapeHtml2(new Date(entry.timestamp).toLocaleString())}</td><td>${escapeHtml2(entry.note || "")}</td></tr>`).join(""), photos = (inspection.photoKeys || []).map((key) => `<img src="${escapeHtml2(`${cloudflareConfig2.apiUrl}/files/${encodeURIComponent(key)}`)}" alt="Inspection photo"/>`).join(""), win = window.open("", "_blank", "noopener");
    if (!win) {
      toast("Allow pop-ups to print the inspection");
      return;
    }
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapeHtml2(inspection.number)} inspection</title><style>${brand.style}body{font:12px/1.55 system-ui,sans-serif;max-width:900px;margin:auto;padding:22px;color:#172029}h2{color:var(--brand)}.summary{display:flex;flex-wrap:wrap;gap:10px;margin:12px 0}.summary b{padding:7px 10px;border:1px solid #d6ddd7}table{width:100%;border-collapse:collapse;margin:10px 0}th,td{padding:6px 8px;border:1px solid #d6ddd7;text-align:left}th{color:#fff;background:var(--brand)}.photos{display:flex;flex-wrap:wrap;gap:8px}.photos img{width:150px;height:110px;object-fit:cover;border:1px solid #d6ddd7}</style></head><body>${brand.header}<h2>Digital Inspection Report: ${escapeHtml2(inspection.number)}</h2><p><b>Customer:</b> ${escapeHtml2(inspection.customer)} \xB7 <b>Vehicle:</b> ${escapeHtml2(inspection.vehicle)} \xB7 <b>Work order:</b> ${escapeHtml2(inspection.workOrderId || "Unlinked")}</p><p><b>Date:</b> ${escapeHtml2(new Date(inspection.createdAt).toLocaleString())}</p><div class="summary"><b>${counts.pass || 0} pass</b><b>${counts.attention || 0} attention</b><b>${counts.fail || 0} fail</b></div>${damage ? `<p><b>Damage zones:</b> ${damage}</p>` : ""}<table><thead><tr><th>Inspection point</th><th>Result</th><th>Note</th></tr></thead><tbody>${itemRows}</tbody></table>${inspection.recommendations ? `<h3>Recommendations</h3><p>${escapeHtml2(inspection.recommendations)}</p>` : ""}${photos ? `<h3>Photos</h3><div class="photos">${photos}</div>` : ""}${approvalRows ? `<h3>Approval history</h3><table><thead><tr><th>Decision</th><th>By</th><th>Date</th><th>Note</th></tr></thead><tbody>${approvalRows}</tbody></table>` : ""}<p>${escapeHtml2(profile.invoiceFooter || "")}</p><button type="button" onclick="window.print()">Print inspection</button></body></html>`);
    win.document.close();
  };
  ...
  function openCustomerEditor(key) {
    ...
    if (!customer) return;
    showModal(`<form class="modal" id="customer-edit-form"><div class="modal-head"><h2>Edit customer</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Name *<input name="name" value="${escapeHtml2(customer.name || "")}" required/></label><label>Phone<input name="phone" value="${escapeHtml2(customer.phone || "")}"/></label><label class="full">Email<input name="email" type="email" value="${escapeHtml2(customer.email || "")}"/></label><label>Billing address<textarea name="billingAddress">${escapeHtml2(customer.billingAddress || "")}</textarea></label><label>Billing notes<textarea name="billingNotes">${escapeHtml2(customer.billingNotes || "")}</textarea></label></div></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">${icon("save", 14)} Save customer</button></div></form>`);
  ...
  invoices = function() {
    const q = query.toLowerCase(), rows = state.invoices.filter((invoice) => !q || Object.values(invoice).join(" ").toLowerCase().includes(q)).map((invoice) => {
      const paid = invoicePaid(invoice), balance = invoiceBalance(invoice), bd = invoiceTaxBreakdown(invoice), key = escapeHtml2(invoiceRecordKey(invoice));
      return `<tr><td class="mono"><b>${escapeHtml2(invoice.number)}</b></td><td class="mono">${escapeHtml2(invoice.ro || "")}</td><td><b>${escapeHtml2(invoice.customer)}</b></td><td>${escapeHtml2(invoice.date || "")}</td><td>${escapeHtml2(invoice.due || "")}</td><td>${balance === 0 ? `<span class="badge paid">Paid</span>` : badge(invoice.status)}</td><td><b>${money(invoice.amount)}</b><small>Subtotal ${money(bd.subtotal)} \xB7 Tax ${money(bd.tax)} (${bd.taxRate}%)</small><small>Paid ${money(paid)} \xB7 Balance ${money(balance)}</small><div class="invoice-payments"><button class="mini-action" data-print-invoice="${escapeHtml2(invoice.number)}">${icon("printer", 13)} Print</button><button class="mini-action" data-edit-invoice="${key}">${icon("pencil", 13)} Edit</button><button class="mini-action danger" data-delete-invoice="${key}">${icon("trash-2", 13)} Delete</button>${balance > 0 ? `<button class="mini-action" data-record-payment="${escapeHtml2(invoice.number)}" data-method="cash">${icon("banknote", 13)} Cash</button><button class="mini-action" data-record-payment="${escapeHtml2(invoice.number)}" data-method="processor">${icon("credit-card", 13)} Card receipt</button><button class="mini-action invoice-pay" data-pay-invoice="${escapeHtml2(invoice.number)}">${icon("external-link", 13)} Pay online</button>` : ""}</div></td></tr>`;
    }).join("");
  ...
  operationsInventory = function() {
    const inventory = operationsInventoryCore(), vendorRows = state.vendors.map((vendor) => `<tr><td><b>${escapeHtml2(vendor.name)}</b><small>${escapeHtml2(vendor.accountNumber || "No account number")}</small></td><td>${escapeHtml2(vendor.phone || "No phone")}</td><td>${escapeHtml2(vendor.email || "No email")}</td><td>${escapeHtml2(vendor.address || "No address")}</td><td><button class="mini-action" data-edit-vendor="${escapeHtml2(vendor.id)}">${icon("pencil", 13)} Edit</button></td></tr>`).join("");
  ...
  addVendor = function(existing = null) {
    showModal(`<form class="modal" id="vendor-form"><div class="modal-head"><h2>${existing ? "Edit" : "Add"} vendor</h2><button type="button" class="close" data-close>${icon("x")}</button></div><div class="modal-body"><div class="form-grid"><label>Name *<input name="name" value="${escapeHtml2(existing?.name || "")}" required/></label><label>Phone<input name="phone" value="${escapeHtml2(existing?.phone || "")}"/></label><label>Email<input name="email" type="email" value="${escapeHtml2(existing?.email || "")}"/></label><label>Account number<input name="accountNumber" value="${escapeHtml2(existing?.accountNumber || "")}"/></label><label class="full">Address<input name="address" value="${escapeHtml2(existing?.address || "")}"/></label></div></div><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">${icon("save", 14)} Save vendor</button></div></form>`);
  ...
  function homeDashboard() {
    ...
    const attention = model.attention.map((item) => {
      ...
      return `<button class="home-row ${tone}" type="button" ${item.orderId ? `data-order="${escapeHtml2(item.orderId)}"` : `data-route="invoices"`}><span class="home-row-icon">${icon(item.type === "overdue" ? "clock-alert" : item.type === "parts" ? "package" : "user-plus", 16)}</span><span><strong>${escapeHtml2(item.title)}</strong><small>${escapeHtml2(detail)}</small></span>${icon("chevron-right", 14)}</button>`;
    }).join("");
    const todayJobs = model.todayJobs.map((order) => `<button class="home-job" type="button" data-order="${escapeHtml2(order.id)}"><div><span class="mono">${escapeHtml2(order.id)}</span>${badge(order.status)}</div><strong>${escapeHtml2(order.customer)}</strong><small>${escapeHtml2(order.vehicle)}</small><span class="home-job-meta">${icon("clock-3", 12)}${escapeHtml2(order.promise || "Unscheduled")} \xB7 ${escapeHtml2(order.tech || "Unassigned")}</span></button>`).join("");
    const appointments = model.todaysAppointments.map((item) => `<article class="home-appointment"><strong>${escapeHtml2(item.time)} \xB7 ${escapeHtml2(item.customer)}</strong><small>${escapeHtml2(item.service)} \xB7 ${escapeHtml2(item.bay || "Unassigned")}</small></article>`).join("");
    const activity = model.activity.map((item) => `<button class="home-row" type="button" ${item.orderId ? `data-order="${escapeHtml2(item.orderId)}"` : 'data-route="invoices"'}><span class="home-row-icon muted">${icon("activity", 15)}</span><span><strong>${escapeHtml2(item.title)} \xB7 ${escapeHtml2(item.customer || "")}</strong><small>${escapeHtml2(item.vehicle || item.meta || "")} \xB7 ${escapeHtml2(label(item.status))}</small></span></button>`).join("");
  ...
  loadInvoicesFromApi = async function() {
    try {
      applyRemoteList("invoices", await apiFetch("/entities/invoices"));
    } catch (error) {
      console.error("Failed to load invoices from API; using local data", error);
    }
  };
  loadExpensesFromApi = async function() {
    try {
      applyRemoteList("expenses", await apiFetch("/entities/expenses"));
    } catch (error) {
      console.error("Failed to load expenses from API; using local data", error);
    }
  };
  ...
  function stampDemoAppointments() {
    const samples = /* @__PURE__ */ new Set(["apt-1048", "apt-1049", "apt-1052"]);
    if (!(state.appointments || []).some((item) => samples.has(item.id))) return;
    const today = /* @__PURE__ */ new Date(), iso = localIsoDate2(today), tomorrow = new Date(today);

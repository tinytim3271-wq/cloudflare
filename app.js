...
(() => {
  ...
  var cloudflareConfig, storageKeys;
  var init_config = __esm({
    "src/shared/config.js"() {
      cloudflareConfig = Object.freeze({
        auth: "mechpro-saas",
        apiUrl: "/api",
        authEndpoints: {
  ...
  var isDesktopApp, DESKTOP_ENTITLEMENT_INTERVAL, platform;
  var init_detect = __esm({
    "src/modules/platform/detect.js"() {
      isDesktopApp = Boolean(window.mechproDesktop);
      DESKTOP_ENTITLEMENT_INTERVAL = 5 * 60 * 1e3;
  ...
  function aiWorkflowForm() {
    const options = visibleOrders().map((order) => `<option value="${order.id}">${order.id} \xB7 ${escapeHtml(order.customer)} \xB7 ${escapeHtml(order.vehicle)}</option>`).join("");
  ...
  function syncStatusBadge() {
    ...
    if (conflicts) return `<span class="sync-pill conflict" title="Offline edits conflict with newer server data">${icon("triangle-alert", 14)} ${conflicts} conflict${conflicts === 1 ? "" : "s"}</span>`;
    if (!online) return `<span class="sync-pill offline" title="Working offline">${icon("wifi-off", 14)} Offline${pending ? ` \xB7 ${pending} queued` : ""}</span>`;
    if (pending) return `<span class="sync-pill pending" title="Waiting to sync offline changes">${icon("refresh-cw", 14)} Syncing ${pending}</span>`;
...
AI workflow: ${aiResult.diagnostics.causes[0]?.cause || "Inspection required"}`.trim();
  ...
  var init_legacy = __esm({
    "src/runtime/legacy.js"() {
      ...
      bindNewOrderEstimatorCore = bindNewOrderEstimator;
      ...
      bindExpandedFeatures = function() {
        ...
        if (profileForm) bindAppearanceControls(profileForm);
      ...
      bindPrintableInvoiceCore = bindEstimateActions;
      bindEstimateActions = function() {
      ...
      bindInvoiceDeleteActionsCore = bindEstimateActions;
      bindEstimateActions = function() {
      ...
      sampleCustomerNames = /* @__PURE__ */ new Set(["Demo Customer A", "Demo Fleet Co", "Demo Customer B", "Demo Customer C", "Demo Customer D", "Demo Retail Co"]);
      onboardingCheckComplete = false;
      customers = function() {
      ...
      operationsInventoryCore = operationsInventory;
      ...
      bindVendorManagementCore = bindShopOperations;
      ...
      settingsDataResetCore = settings;
      settings = function() {
      ...
      bindAgentPhoneSettingsCore = bindExpandedFeatures;
      bindExpandedFeatures = function() {
      ...
      apiFetchAssistantCore = apiFetch;
  ...
  __export(storage_exports, {
    ...
    writeDispatchState: () => writeDispatchState,
    writeJson: () => writeJson,
  ...
  window.__MECHPRO_BOOTSTRAP_READY__ = (async () => {
    ...
    try {
      await Promise.resolve().then(() => (init_legacy(), legacy_exports));
    } finally {
